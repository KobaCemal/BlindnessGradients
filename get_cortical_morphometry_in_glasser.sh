...
do 
    ...
    do 
        ...
        mris_anatomical_stats -a "$SUBJECTS_DIR"/"$sub"/label/"$hemi".glasser-360_mics.annot -t "$SUBJECTS_DIR"/"$sub"/surf/lh.sulc -b "$sub" "$hemi"  > "$SUBJECTS_DIR"/"$sub"/stats/"$hemi".glasser_sulc.stats
    done
done
